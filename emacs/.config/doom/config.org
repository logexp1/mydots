...
(use-package! yaml
  :config
  (add-hook! 'yaml-mode-hook (lambda () (spell-fu-mode 0))))
#+end_src
...
;;   :config
;;   (map! :leader
;;         :desc "k8s" "k" #'kubernetes-overview))
...
(use-package! kubed
  :config
  (kubed-define-resource serviceaccount () :plural nil)
  (kubed-define-resource pvc () :plural nil)
